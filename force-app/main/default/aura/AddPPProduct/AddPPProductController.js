({
        ...
        for(var i=0;i<tmpWrapper.length;i++)
        {
            ...
            if(i == tmpIndex)
            {
                ...
                if(event.getSource().get("v.id") == "discountpercent"){  
                    child.DiscountN__c = (event.getSource().get("v.value") * child.Total_Amount__c)/100;
                    child.After_Discount_Net_Amount__c = child.Total_Amount__c - child.DiscountN__c;
                }
                else{
                    
                    child.Discount__c = (event.getSource().get("v.value")*100 )/ child.Total_Amount__c;
                    child.After_Discount_Net_Amount__c = child.Total_Amount__c - event.getSource().get("v.value");
                }
                console.log('Total '+child.After_Discount_Net_Amount__c);
            }
        }     
        component.set("v.wrapMain.lstWrappTwo", tmpWrapper);  
    },
    nextJs :function(component, event, helper) {
        ...
        if(Istrue == true || tmpMainWrapper !=null){
